...
export default function AdminProduct() {
    ...
    const handleAdd = async () => {
        ...
        if (formValues) {
            try {
                await fetch(`http://localhost:3000/product/save`, {
                    method: 'POST',
                    headers: {
    ...
    const handleEdit = async (id, currentData) => {
        ...
        if (formValues) {
            try {
                await fetch(`http://localhost:3000/product/update/${id}`, {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json'
                    },
    ...
    const handleRemove = async (id) => {
        ...
        if (result.isConfirmed) {
            try {
                await fetch(`http://localhost:3000/product/delete/${id}`, {
                    method: 'DELETE'
                });
                MySwal.fire({
                    title: 'Deleted!',
    ...
    return (
        <div className="p-20 bg-white rounded-lg shadow-lg">
            <div className="relative overflow-x-auto l:rounded-lg">
                <h1 className="pt-2 pl-1 text-3xl text-blue-500">Product Table: {currentDateTime}</h1>
                ...
                <table className="w-full text-sm text-left text-gray-500">
                    ...
                    <tbody>
                        {currentRows.map((product) => (
                            <tr key={product.id} className="text-white bg-blue-500 border-b border-blue-400 hover:bg-blue-400">
                                ...
                                <td className="px-6 py-3">
                                    <button className="px-3 py-1 mr-2 font-medium text-white bg-yellow-500 rounded hover:bg-yellow-600" onClick={() => handleEdit(product.id, product)}>
                                        Edit
                                    </button>
                                    <button className="px-3 py-1 font-medium text-white bg-red-500 rounded hover:bg-red-600" onClick={() => handleRemove(product.id)}>
                                        Remove
                                    </button>
                                </td>
                            </tr>
                ...
                <nav className="flex items-center justify-between pt-2" aria-label="Table navigation">
                    ...
                    <ul className="inline-flex h-10 pr-10 -space-x-px text-sm rtl:space-x-reverse">
                        <li>
                            <button onClick={handlePrevPage} className="flex items-center justify-center h-8 px-3 leading-tight text-gray-500 bg-white border border-gray-300 rounded-l-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white" disabled={currentPage === 1}>
                                Previous
                            </button>
                        </li>
                        ...
                        <li>
                            <button onClick={handleNextPage} className="flex items-center justify-center h-8 px-3 leading-tight text-gray-500 bg-white border border-gray-300 rounded-r-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white" disabled={currentPage === totalPages}>
                                Next
                            </button>
                        </li>
                    </ul>
